...
async def _check_token_can_assign_role(
    ...
    Layer 3 â€” API tokens never grant Admin or Maintainer. Elevated roles must
    be assigned through interactive admin flows so a leaked token cannot mint
    org admins.

    Layer 2 â€” The token's creator must still be a member of the org and must
...
async def issue_user_token(
    ...
    # Issue a short-lived token (1 hour) for headless use â€” shorter than the
...
async def get_user_trail_detail(
    ...
    """Build a full trail breakdown for a user â€” every chapter + every activity
...
async def provision_user(
    ...
    Designed for SSO/JIT provisioning â€” email is auto-verified and the user
    ...
    if existing_user:
        # Email matches an existing account â€” treat this as "attach to org"
...
def _validate_magic_link_redirect(redirect_to: Optional[str]) -> Optional[str]:
    ...
    URL â€” only allows paths like "/course/foo" that resolve on the same host
...
async def consume_magic_link_token(
    ...
    # have none â€” let them through; the JWT exp (max 15 min) bounds them.
...
async def update_user_profile(
    ...
    """Update a user's profile fields. Org-scoped â€” user must be a member."""
...
async def export_user_data(
    ...
    Only returns data that belongs to the token's organization â€” other-org
...
async def anonymize_user(
    ...
    *caller's* org â€” tokens the user created in other orgs are NOT touched.
