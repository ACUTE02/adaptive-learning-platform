...
class TestUnicodeNormalization:
    ...
    The combined form (NFC) of "cafÃ©" is 4 code points; the decomposed form
    ...
    def test_build_like_pattern_normalizes_and_escapes(self):
        ...
        pattern = build_like_pattern("cafeÌ")
        assert pattern == "%cafÃ©%"
    ...
    def test_build_like_pattern_handles_emoji(self):
        ...
        family = "\U0001f468â€\U0001f469â€\U0001f467"
...
class TestSearchUnicodeMatching:
    ...
    async def test_search_matches_nfd_query_to_nfc_stored(
        ...
        # Stored as NFC ("cafÃ©"); user query is NFD ("cafe" + U+0301).
        await _make_collection(
            db, org, id=72,
            name="cafÃ© club",
        ...
        with patch(
            ...
            result = await search_across_org(
                mock_request, anonymous_user, "test-org", "cafeÌ", db
            )

        names = [c.name for c in result.collections]
        assert "cafÃ© club" in names
