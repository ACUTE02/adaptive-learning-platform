...
class TestImportHelpers:
    ...
    async def test_analyze_import_package_skips_path_traversal_and_extracts_dirs(
        ...
        with zf.ZipFile(buf, "w", compression=zf.ZIP_STORED) as z:
            # directory entry â†’ covers line 162 (call 1)
            dir_info = zf.ZipInfo("c1/subdir/")
            dir_info.external_attr = 0o40755 << 16
            z.writestr(dir_info, "")
            # pure dot-dot entry â†’ sanitize_path returns '' â†’ covers line 153 (call 2)
            z.writestr("..", b"evil")
            # valid files (calls 3 and 4)
            z.writestr("manifest.json", json.dumps(manifest))
            z.writestr("c1/course.json", json.dumps(course_data))
            # extra entry whose path will be made to escape extract_dir â†’ covers line 159 (call 5)
            z.writestr("c1/extra.txt", b"extra")

        package_bytes = buf.getvalue()

        # Patch sanitize_path so that:
        # - call #2 (the ".." entry) returns "" â†’ covers line 153
        # - call #3 (manifest.json) returns "../../escape" â†’ covers line 159
        ...
        def _patched_sanitize(path):
            call_count["n"] += 1
            if call_count["n"] == 2:
                return ""              # empty â†’ line 153 fires
            if call_count["n"] == 5:
                return "../../escape"  # escapes extract_dir â†’ line 159 fires
    ...
    async def test_analyze_import_package_rejects_symlink_entry(
        ...
        symbolic links â€” extracting them lets a malicious archive redirect
        ...
        with zf.ZipFile(buf, "w", compression=zf.ZIP_STORED) as z:
            ...
            # 16 bits of external_attr â€” matches the convention zipfile uses.
