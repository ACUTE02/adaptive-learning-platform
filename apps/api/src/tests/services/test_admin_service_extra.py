...
# ---------------------------------------------------------------------------
# Lines 986, 995 â€” consume_magic_link_token
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_consume_magic_link_token_incomplete_payload_raises_410(db):
    """Line 986: payload has purpose=magic_link but no sub â†’ 410."""
...
async def test_consume_magic_link_token_ghost_user_raises_410(db):
    """Line 995: valid sub + org_id but user doesn't exist in DB â†’ 410."""
    with patch("src.security.auth.decode_jwt") as mock_decode:
...
# ---------------------------------------------------------------------------
# Line 1125 â€” list_course_enrollments course not found
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Lines 847-848, 852-853 â€” remove_user_from_org_admin exception swallowing
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_remove_user_from_org_invalidate_cache_raises_is_swallowed(db, org, admin_role, user_role):
    """Lines 847-848: _invalidate_session_cache raises â†’ swallowed, function succeeds."""
    token_user = _make_token_user(org.id)
...
async def test_remove_user_from_org_decrease_feature_usage_raises_is_swallowed(db, org, admin_role, user_role):
    """Lines 852-853: decrease_feature_usage raises â†’ swallowed, function succeeds."""
    token_user = _make_token_user(org.id)
...
# ---------------------------------------------------------------------------
# Lines 1509-1510 â€” update_user_profile _invalidate_session_cache swallowed
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_update_user_profile_invalidate_cache_raises_is_swallowed(db, org, user_role):
    """Lines 1509-1510: _invalidate_session_cache raises â†’ swallowed."""
    token_user = _make_token_user(org.id)
...
# ---------------------------------------------------------------------------
# Line 1529 â€” change_user_role role belongs to wrong org
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_change_user_role_role_wrong_org_raises_403(db, org, user_role):
    """Line 1529: role.org_id != token_user.org_id â†’ 403."""
    token_user = _make_token_user(org.id)
...
# ---------------------------------------------------------------------------
# Lines 1561-1562 â€” change_user_role _invalidate_session_cache swallowed
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_change_user_role_invalidate_cache_raises_is_swallowed(db, org, admin_role, user_role):
    """Lines 1561-1562: _invalidate_session_cache raises â†’ swallowed, role updated."""
    token_user = _make_token_user(org.id)
...
# ---------------------------------------------------------------------------
# Line 1836 â€” bulk_unenroll_users user with no TrailRun â†’ not_enrolled
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_bulk_unenroll_users_user_not_enrolled_goes_to_not_enrolled(db, org, course, user_role):
    """Line 1836: user_id with no TrailRun â†’ appended to not_enrolled."""
    token_user = _make_token_user(org.id)
    user = await _create_user(db, user_id=90, username="unenroll90", email="unenroll90@test.com")
    await _add_user_to_org(db, user, org, role_id=user_role.id)

    # Do NOT create a TrailRun for this user â€” they are not enrolled
...
# ---------------------------------------------------------------------------
# Lines 1979-1980 â€” anonymize_user _invalidate_session_cache swallowed
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_anonymize_user_invalidate_cache_raises_is_swallowed(db, org, user_role):
    """Lines 1979-1980: _invalidate_session_cache raises â†’ swallowed."""
    token_user = _make_token_user(org.id)
...
# ---------------------------------------------------------------------------
# Line 827 â€” remove_user_from_org_admin: user passes _get_user_in_org but
...
# ---------------------------------------------------------------------------
# Line 1538 â€” change_user_role: user passes _get_user_in_org but no
...
# ---------------------------------------------------------------------------
# Line 1836 â€” bulk_unenroll_users: enrolled user with TrailSteps gets steps
