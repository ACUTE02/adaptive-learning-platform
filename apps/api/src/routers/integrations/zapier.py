...
The token carries the organization scope â€” no ``org_id`` appears in the URL.
...
# not enforced â€” importing it here is deliberate to avoid duplicating the
# logic across files.
...
@router.post(
    ...
    responses={
        201: {"description": "Subscription created â€” webhook endpoint registered for the given event.", "model": ZapierSubscriptionResponse},
...
async def zapier_create_subscription(
    ...
    # The signing secret is never returned to Zapier â€” Zapier's Catch Hook
