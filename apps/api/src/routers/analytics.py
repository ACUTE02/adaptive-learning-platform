...
from src.db.engine_models import CampaignModuleDB, Assessment, Campaign
from src.services.analytics_engine import calculate_retention_probability
...
async def _execute_tinybird_query(
    ...
    # Tinybird returns {"data": [...], "rows": N, "meta": [...]} â€” same shape as frontend expects.
...
async def _verify_org_admin(user_id: int, org_id: int, db_session: AsyncSession) -> None:
    ...
    to the actual organization being accessed â€” not any org the user belongs to.
...
# -------------------------------------------------------------------
# GET /status â€” Whether analytics (Tinybird) is configured
# -------------------------------------------------------------------
@router.get(
    "/status",
...
# -------------------------------------------------------------------
# POST /events â€” Frontend event proxy
# -------------------------------------------------------------------
@router.post(
    "/events",
...
async def ingest_frontend_event(
    ...
    # Sanitize seconds_spent â€” cap at 4 hours, reject negatives
...
# -------------------------------------------------------------------
# GET /dashboard/detail/{query_name} â€” Tinybird + PostgreSQL enrichment
# -------------------------------------------------------------------
@router.get(
...
# -------------------------------------------------------------------
# GET /dashboard/{query_name} â€” Run analytics query via Tinybird
# -------------------------------------------------------------------
@router.get(
...
async def query_dashboard(
    ...
    if query_name in ADVANCED_QUERIES:
        ...
        if bypass is None:  # SaaS mode â€” check plan
...
# -------------------------------------------------------------------
# GET /dashboard/db/{query_name} â€” PostgreSQL-based queries
# -------------------------------------------------------------------
@router.get(
...
def _build_sql(
    ...
    if course_uuid is not None:
        # Re-validate even if caller already did â€” defense in depth
...
# -------------------------------------------------------------------
# GET /dashboard/course/detail/{query_name} â€” Course-level detail with enrichment
# -------------------------------------------------------------------
@router.get(
...
# -------------------------------------------------------------------
# GET /dashboard/course/{query_name} â€” Course-level analytics (Pro only)
# -------------------------------------------------------------------
@router.get(
...
# -------------------------------------------------------------------
# GET /export â€” Export analytics data as JSON or CSV
# -------------------------------------------------------------------
@router.get(
    "/export",
...
# -------------------------------------------------------------------
# GET /plan-info â€” Returns analytics tier for the org
# -------------------------------------------------------------------
@router.get(
    "/plan-info",
...
# -------------------------------------------------------------------
# GET /overview - Predictive Analytics
# -------------------------------------------------------------------
@router.get(
    "/overview",
    summary="Get custom predictive analytics overview",
)
async def get_analytics_overview(
    ...
    statement = (
        ...
        .join(Campaign)
        .where(Campaign.user_id == user_email, CampaignModuleDB.requires_remediation == True)
    )
    if campaign_id:
        statement = statement.where(Campaign.id == campaign_id)
        
    modules = (await db_session.execute(statement)).scalars().all()
    
    remediation_modules = [{"id": m.id, "title": m.title} for m in modules]
    
    active_modules_stmt = (
        select(CampaignModuleDB)
        .join(Campaign)
        .where(Campaign.user_id == user_email, CampaignModuleDB.status != 'locked')
    )
    if campaign_id:
        active_modules_stmt = active_modules_stmt.where(Campaign.id == campaign_id)
    ...
    for module in active_modules:
        raw_score = float(module.current_retention_score or 100.0)
...
# -------------------------------------------------------------------
# GET /chart-data - Predictive Analytics Charting
# -------------------------------------------------------------------
@router.get(
    "/chart-data",
    summary="Get analytics chart data for a user",
)
async def get_chart_data(
    ...
    statement = (
        select(Campaign, CampaignModuleDB)
        .join(CampaignModuleDB, CampaignModuleDB.campaign_id == Campaign.id)
        .join(Assessment, Assessment.module_id == CampaignModuleDB.id)
        .where(Campaign.user_id == user_email)
        .where(Assessment.status == 'completed')
        .where(CampaignModuleDB.status != 'locked')
        .order_by(CampaignModuleDB.id)
    )
    results = (await db_session.execute(statement)).all()
    
    # Group modules by campaign
    campaigns_dict = {}
    for campaign, module in results:
        if campaign.id not in campaigns_dict:
            campaigns_dict[campaign.id] = {
                "campaign_id": campaign.id,
                "campaign_name": campaign.title,
                "modules": []
            }
            
        existing = any(m["module_id"] == module.id for m in campaigns_dict[campaign.id]["modules"])
        if not existing:
            raw_score = float(module.current_retention_score or 100.0)
            display_score = int(raw_score * 100) if raw_score <= 1.0 else int(raw_score)
            
            campaigns_dict[campaign.id]["modules"].append({
                "module_id": module.id,
                "name": module.title,
                "retention_score": display_score
            })
