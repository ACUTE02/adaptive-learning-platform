...
from src.db.engine_models import Course
...
def _is_non_saas() -> bool:
    """Check if deployment is in a non-SaaS mode (EE or OSS) â€” disables plan-based limits."""
...
async def check_limits_with_usage(
    ...
    # Unlimited (limit=0) â€” no usage check needed
    if resolved["limit"] == 0:
        return True
...
async def check_feature_access(
    ...
    1. If OSS mode is enabled â†’ allow access
    2. If the organization's plan meets the required level â†’ allow access
    3. Otherwise â†’ deny access with 403
...
async def reserve_ai_credit(
    ...
    try:
        ...
        # Fail closed â€” prefer quota denial over silent over-use.
        raise HTTPException(
            status_code=503,
