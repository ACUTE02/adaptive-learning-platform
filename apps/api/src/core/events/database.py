...
if is_testing:
    ...
    # Passing name="" makes named="" (falsy) â†’ asyncpg uses stmt_name='' (unnamed slot).
    #
    ...
    #   statement_cache_size=0           â†’ disable asyncpg's own per-connection LRU
    #   prepared_statement_name_func=""  â†’ force unnamed prepared statements
    #   prepared_statement_cache_size=0  â†’ disable SQLAlchemy's adapter-level LRU
    _connect_args = {
    ...
    if is_pooled:
        ...
        logging.info("DB engine: detected connection pooler â€” using small client-side pool.")
    else:
        engine_kwargs = dict(
            pool_pre_ping=True,
            pool_size=20,
...
def _register_cache_invalidation_hooks():
    ...
    # â”€â”€ Mapper-level events: fire per-object during flush â”€â”€
...
async def connect_to_db(app: FastAPI):
    async with engine.begin() as conn:
        # Enable pgvector extension for vector similarity search (optional â€” RAG feature)
        try:
            from sqlalchemy import text
            await conn.execute(text("CREATE EXTENSION IF NOT EXISTS vector"))
        except Exception as e:
            logging.warning(
                "pgvector extension not available â€” RAG features will be disabled. "
