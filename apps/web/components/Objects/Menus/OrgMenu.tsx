      ...
      >
        <div className="flex items-center justify-between w-full max-w-(--breakpoint-2xl) mx-auto px-4 sm:px-6 lg:px-8 h-full">
          ...
          <div className="flex items-center space-x-2">
            {/* Boards (Exam Scratchpad) */}
            {rf?.boards?.enabled && (isAdmin || pathname?.includes('/dash/assignments') || pathname?.includes('/dash/sniper') || pathname?.includes('/dash/campaign')) && (
            ...
            {/* AI Copilot (AI Remediation Tutor) */}
            {rf?.ai?.enabled && config?.admin_toggles?.ai?.copilot_enabled !== false && (isAdmin || pathname?.includes('/dash/sniper') || pathname?.includes('/dash/campaign')) && (
              <AuthenticatedClientElement checkMethod="authentication">
                <div className="hidden md:flex">
                  <CopilotMenuButton
                    orgslug={orgslug}
    ...
    <DropdownMenu onOpenChange={setIsOpen}>
          ...
          <TooltipTrigger asChild>
            <DropdownMenuTrigger asChild>
              <button
                ...
                aria-label="AI Remediation Tutor"
              >
      ...
      <DropdownMenuContent align="end" className="w-64">
        <DropdownMenuLabel className="flex items-center gap-2">
          ...
          <span>AI Remediation Tutor</span>
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
